# from https://github.com/facebookresearch/DigitalTwinCatalog/blob/main/neural_pbir/pbir/models/utils_sg.py
...
def Envmap2SG(gt_envmap, numLgtSGs=128, N_iter=1000, fixed_lobe=False, debug=False):
    ...
    for step in range(N_iter):
        ...
        if step % 10 == 0 and debug:
            ...
            imageio.imwrite(os.path.join("output", "log_im_{}.png".format(step)), im)
...
def main():
    # load data
    envmap_filepath = "/path/to/datasets/Environment_Maps/high_res_envmaps_1k/sunset.hdr"
    envmap = mi.Bitmap(envmap_filepath)
    ...
    original_path = os.path.join(output_dir, "original_envmap.exr")
    mi.Bitmap(envmap).write(original_path)

    gt_envmap = torch.FloatTensor(np.array(envmap)).to(device)
    
    H = gt_envmap.shape[0]
    W = gt_envmap.shape[1]

    trained_sgs = Envmap2SG(
            gt_envmap, 
            numLgtSGs=num_sgs, 
            N_iter=n_iterations, 
            fixed_lobe=False, 
    ...
    reconstructed_envmap = SG2Envmap(trained_sgs, H, W)
    reconstructed_envmap_np = reconstructed_envmap.detach().cpu().numpy()

    sgs_path = os.path.join(output_dir, "reconstructed_lgtSGs.npy")
    np.save(sgs_path, trained_sgs.detach().cpu().numpy())
    
    reconstructed_hdr_path = os.path.join(output_dir, "reconstructed_envmap.exr")
    mi.Bitmap(mi.Bitmap(reconstructed_envmap_np)).write(reconstructed_hdr_path)
