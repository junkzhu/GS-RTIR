...
if __name__ == "__main__":
    ...
    #params['shape.opacities'] = dr.full(mi.Float, 0.2, n_ellipsoids)

    #clear sh & normal
    m_sh_coeffs = params['shape.sh_coeffs'].shape[0] // n_ellipsoids
    m_normals = params['shape.normals'].shape[0] // n_ellipsoids
    #params['shape.sh_coeffs'] = dr.full(mi.Float, 0.1, n_ellipsoids * m_sh_coeffs)
    ...
    for i in pbar:
        ...
        for idx, sensor in dataset.get_sensor_iterator():
            ...
            fake_normal_loss = lnormal_sqr(ref_normal, fake_normal_img, normal_mask_flat) / dataset.batch_size
