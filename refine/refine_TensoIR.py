...
class RefineTensoIR(RefineBase):
    """Refinement class for TensoIR dataset"""
    ...
    def calculate_loss(self, idx, sensor, img, aovs, ref_img):
        ...
        # Get reference normal image
        ref_normal = self.dataset.ref_normal_images[idx][sensor.film().crop_size()[0]]
        ...
        normal_mae_val = lmae(ref_normal, normal_img, normal_mask.squeeze()) / self.dataset.batch_size
        
        return view_loss, total_loss, rgb_psnr_val, normal_mae_val

if __name__ == "__main__":
    refine = RefineTensoIR()
    refine.run()