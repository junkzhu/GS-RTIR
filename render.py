...
def load_scene_config(gaussians_attributes, envmap_init_path, optimize_envmap):
    ...
    if optimize_envmap:
        if args.spherical_gaussian:
            ...
            # Use OUTPUT_ENVMAP_DIR and iteration number to construct envmap path
            envmap_iter = 799
            envmap_filename = join(OUTPUT_ENVMAP_DIR, f'{envmap_iter:04d}.exr')
            scene_config['envmap'] = {
                'type': 'envmap',
...
def render_relight_images(gaussians, dataset, scene_dict, params):
    ...
    for envmap in envmaps:
        ...
        for idx, sensor in relight_pbar:
            ...
            buffer = mi.render(scene_dict, params=params, sensor=sensor, spp=args.render_spp)
            img, aovs = unpack_buffer(buffer)  # img is linear space
...
def render_materials(dataset, scene_dict):
    ...
    for idx, sensor in pbar:
        ...
        buffer = mi.render(scene_dict, sensor=sensor, spp=args.render_spp) #img is linear space
        ...
        ref_albedo_img = mi.Bitmap(dataset.ref_albedo_images[idx][sensor.film().crop_size()[0]])
        write_bitmap(join(OUTPUT_RENDER_DIR, f'{idx:02d}_albedo_ref' + ('.png')), ref_albedo_img)

        write_bitmap(join(OUTPUT_RENDER_DIR, f'{idx:02d}_roughness' + ('.png')), roughness_bmp)
        ref_roughness_img = mi.Bitmap(dataset.ref_roughness_images[idx][sensor.film().crop_size()[0]])
        write_bitmap(join(OUTPUT_RENDER_DIR, f'{idx:02d}_roughness_ref' + ('.png')), ref_roughness_img)
        ...
        ref_albedo_for_list = mi.Bitmap(dataset.ref_albedo_images[idx][sensor.film().crop_size()[0]])
        ref_albedo_list.append(ref_albedo_for_list)

        rgb_bmp = resize_img(mi.Bitmap(img),dataset.target_res)
        write_bitmap(join(OUTPUT_RENDER_DIR, f'{idx:02d}' + ('.png')), rgb_bmp)
        ref_rgb_img = mi.Bitmap(dataset.ref_images[idx][sensor.film().crop_size()[0]])
        write_bitmap(join(OUTPUT_RENDER_DIR, f'{idx:02d}_ref' + ('.png')), ref_rgb_img)
...
if __name__ == "__main__":
    ...
    # If base renders already exist, skip re-render and only do relight
    if args.relight and not args.only_relight:
        try:
            if os.path.isdir(OUTPUT_RENDER_DIR) and any(
                f.endswith(".png") and re.match(r"^\d\d\.png$", f)
                for f in os.listdir(OUTPUT_RENDER_DIR)
            ):
                args.only_relight = True
        except Exception:
            pass

    if args.relight and args.only_relight:
        # Use rescaled ply if available (created by previous render.py run)
        relight_ply = args.ply_path.replace(".ply", "_rescaled.ply")
        if os.path.isfile(relight_ply):
            args.ply_path = relight_ply
