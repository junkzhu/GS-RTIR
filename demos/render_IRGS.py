...
OUTPUT_DIR = join(os.path.dirname(os.path.abspath(__file__)), 'outputs', 'lego')
...
def get_model_configs():
    ...
    return [
        ...
        {
            'ply_path': './demos/scenes/TensoIR/mic.ply',
            'rotation': [0, 0, 180],
            'scale': [1.0, 1.0, 1.0],
            'translation': [0.0, -1.5, 0.4]
        },
    ]

def render_lego(idx, render_spp, envmap_path, sensor):
...
if __name__ == "__main__":
    ...
    with time_measure("Loading dataset"):
        #sensors = load_sensors('/path/to/datasets/TensoIR/lego/', 800, 600, 120)
        sensors = make_half_topdown_orbit_sensors_zup(
            resx=1200, resy=800, fov=49.1,
            n_frames=180,
            center=(0, 0, 0),
            radius=10,
            height=5.0,          # Larger height means more top-down view
    ...
    for idx in trange(len(sensors), desc="Rendering", unit="spp"):
        render_lego(idx, render_spp, envmap_path, sensors[idx])
