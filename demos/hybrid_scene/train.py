...
def save_render_results(i, img, ref_img, aovs):
    rgb_bmp = resize_img(mi.Bitmap(img), [600, 600])
    rgb_ref_bmp = resize_img(mi.Bitmap(ref_img), [600, 600])
    albedo_bmp = resize_img(mi.Bitmap(aovs['albedo']), [600, 600])
    roughness_bmp = resize_img(mi.Bitmap(aovs['roughness']), [600, 600])
    metallic_bmp = resize_img(mi.Bitmap(aovs['metallic']), [600, 600])

    indirect_bmp = resize_img(mi.Bitmap(aovs['indirect_light']), [600, 600])
    direct_bmp = resize_img(mi.Bitmap(aovs['direct_light']), [600, 600])

    write_bitmap(join(OUTPUT_DIR, './rgb/', f'opt-{i:04d}' + ('.png')), rgb_bmp)
    write_bitmap(join(OUTPUT_DIR, './rgb/', f'opt-{i:04d}_ref' + ('.png')), rgb_ref_bmp)
    write_bitmap(join(OUTPUT_DIR, './albedo/', f'opt-{i:04d}_albedo' + ('.png')), albedo_bmp)
    write_bitmap(join(OUTPUT_DIR, './roughness/', f'opt-{i:04d}_roughness' + ('.png')), roughness_bmp)
    write_bitmap(join(OUTPUT_DIR, './metallic/', f'opt-{i:04d}_metallic' + ('.png')), metallic_bmp)
    write_bitmap(join(OUTPUT_DIR, './indirect_light/', f'opt-{i:04d}_indirect_light' + ('.png')), indirect_bmp)
    write_bitmap(join(OUTPUT_DIR, './direct_light/', f'opt-{i:04d}_direct_light' + ('.png')), direct_bmp)
...
def update_sensors(sensor, i, res):
    if i == 199:
        set_sensor_res(sensor, [300, 300])
        res = 300
    if i == 399:
        set_sensor_res(sensor, [600, 600])
        res = 600
    return res

def train_loop(sensor, sensor_render, scene_dict, params, opt, ref_images, training_spp):
    seed = 0
    ...
    pbar = tqdm.tqdm(range(999999))
    for i in pbar:
        loss, rgb_psnr, albedo_psnr, roughness_mse, metallic_mse = mi.Float(0.0), mi.Float(0.0), mi.Float(0.0), mi.Float(0.0), mi.Float(0.0)
        buffer = mi.render(scene_dict, sensor=sensor, params=params, spp=training_spp * 4, spp_grad=training_spp, seed=seed, seed_grad=seed + 1)
        ...
        # Convert numpy array to TensorXf
        ref_img = mi.TensorXf(ref_img)
        ...
        # Render Results
        if i % 100 == 0:
            buffer = mi.render(scene_dict, sensor=sensor_render, params=params, spp=1000, seed=seed, seed_grad=seed + 1)
            dr.eval(buffer)
            img, aovs = unpack_buffer(buffer)
            save_render_results(10000+i, img, ref_img, aovs)
    
    img, aovs = mi.render(scene_dict, sensor=sensor_render, params=params, spp=1000, seed=seed, seed_grad=seed + 1)
    save_render_results(99999, img, ref_img, aovs)
...
if __name__ == "__main__":
    ...
    # Training loop
    train_loop(sensor_train, sensor_render, scene_dict, params, opt, ref_images, training_spp)
