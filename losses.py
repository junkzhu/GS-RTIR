...
from utils import linear_to_srgb, convert_depth_to_normal

# ---------- Per-term implementations ----------
def compute_losses(img, aovs, ref_imgs, priors_imgs, sensor, i, train_conf):
    """Compute all losses for the current iteration"""
    view_loss = compute_view_loss(ref_imgs['rgb'], img)
    priors_loss = compute_priors_losses(aovs, priors_imgs)
    tv_loss = compute_tv_losses(img, ref_imgs['rgb'], aovs)
    normal_loss, normal_mask = compute_normal_loss(aovs, sensor)

    # warmup or standard
    is_warmup = i < train_conf.optimizer.warmup_iterations
    weights = train_conf.optimizer.loss_weights.warmup if is_warmup else train_conf.optimizer.loss_weights.standard
    
    total_loss = weights.view * view_loss
    total_loss += weights.normal * normal_loss
    total_loss += weights.tv * tv_loss
    total_loss += weights.priors * priors_loss
    
    return total_loss, normal_mask

def compute_view_loss(ref_img, img):
    """View (RGB) reconstruction loss."""
    return l1(ref_img, img, convert_to_srgb=True)

def compute_priors_losses(aovs, priors_imgs):
    """Priors loss for albedo, roughness, and normal."""
    albedo_priors_loss = l2(aovs['albedo'], priors_imgs['albedo'])
    roughness_priors_loss = l2(aovs['roughness'], priors_imgs['roughness'])
    normal_priors_loss = l2(aovs['normal'], priors_imgs['normal'])
    return albedo_priors_loss + roughness_priors_loss + normal_priors_loss

def compute_tv_losses(img, ref_img, aovs):
    """Total variation losses (RGB + material)."""
    rgb_tv_loss = TV(ref_img, img)
    material_tv_loss = TV(ref_img, dr.concat([aovs['albedo'], aovs['roughness'], aovs['metallic']], axis=2))
    return rgb_tv_loss + material_tv_loss

def compute_normal_loss(aovs, sensor):
    """Normal consistency loss from depth-derived normals."""
    normal_norm = np.linalg.norm(aovs['normal'], axis=2, keepdims=True)
    normal_mask = normal_norm > 0.1
    normal_mask_flat = np.reshape(normal_mask, (-1, 1)).squeeze()
    fake_normal_img = convert_depth_to_normal(aovs['depth'], sensor)
    normal_loss = lnormal_sqr(aovs['normal'], fake_normal_img, normal_mask_flat)
    return normal_loss, normal_mask
